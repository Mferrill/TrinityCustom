...
#define GHBUY_COST 10000000  //1000 g.
#define GHSELL_COST 5000000       // 500 g
...
bool OnGossipHello(Player *player, Creature* pCreature)
{
	...
	if (isPlayerGuildLeader(player))
	{
		...
		if (isPlayerHasGuildhouse(player, pCreature))
		{
			...
			player->PlayerTalkClass->GetGossipMenu().AddMenuItem(-1, ICON_GOSSIP_GOLD, MSG_GOSSIP_SELL, GOSSIP_SENDER_MAIN, ACTION_SELL_GUILDHOUSE, MSG_CODEBOX_SELL, 0, true);
		}
	}
	player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, pCreature->GetGUID());
	return true;
